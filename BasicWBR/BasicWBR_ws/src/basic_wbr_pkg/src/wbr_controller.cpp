...
int main(int argc, char * argv[])
{
    ...
    ros::Publisher Right_Hip_Joint_Pos_Cmd_Pub = nt.advertise<std_msgs::Float64>("/robot_wbr/Rh_ctrl/command",10);
    ros::Publisher Left_Knee_Joint_Pos_Cmd_Pub = nt.advertise<std_msgs::Float64>("/robot_wbr/Lk_ctrl/command",10);
    ros::Publisher Right_Knee_Joint_Pos_Cmd_Pub = nt.advertise<std_msgs::Float64>("/robot_wbr/Rk_ctrl/command",10);
    
    ros::Publisher LeftWheelEffortCmdPub = nt.advertise<std_msgs::Float64>("/robot_wbr/Lw_ctrl/command",10);
    ros::Publisher RightWheelEffortCmdPub = nt.advertise<std_msgs::Float64>("/robot_wbr/Rw_ctrl/command",10);
    ...
    std_msgs::Float64 Lh_Pos_CmdMsg; //控制指令
    std_msgs::Float64 Rh_Pos_CmdMsg; //控制指令
    std_msgs::Float64 Lk_Pos_CmdMsg; //控制指令
    std_msgs::Float64 Rk_Pos_CmdMsg; //控制指令
    std_msgs::Float64 RobotPitch;   //解算后的角度


    Lh_Pos_CmdMsg.data = 0 ; //控制指令
    Rh_Pos_CmdMsg.data = 0; //控制指令
    Lk_Pos_CmdMsg.data = 0; //控制指令
    Rk_Pos_CmdMsg.data = 0;
